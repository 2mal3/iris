...
func Main(config Config) {
	...
	for _, inputFolderPath := range config.InputPaths {
		...
		err = filepath.WalkDir(
			inputFolderPath,
			func(srcFilePath string, srcFileInfo os.DirEntry, err error) error {
				return walk(srcFilePath, srcFileInfo, err, config)
			},
		)
		if err != nil {
			slog.Error(err.Error())
...
func walk(srcFilePath string, srcFileInfo os.DirEntry, err error, config Config) error {
	if err != nil {
		slog.Error(err.Error())
		return nil
	}
